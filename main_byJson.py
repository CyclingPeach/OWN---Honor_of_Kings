...
'''
    通过 json 文件获取英雄信息（名称和url链接）
'''
def get_heroInfo_byJson():
    with open('herolist.json', encoding='utf-8') as json_info:
        hero_info = json.load(json_info)
        hero_name_list = []
        hero_url_list = []
        herolist_url_herd = 'https://pvp.qq.com/web201605/herodetail/'
        for i in hero_info:
            hero_name_list.append(i.get('cname'))
            hero_url_list.append(herolist_url_herd + str(i.get('ename')) + '.shtml')
        print(hero_name_list)
        print(hero_url_list)
    return list(zip(hero_name_list, hero_url_list))
...
def save_hero_info():
    ...
    for one_hero_url in get_heroInfo_byJson():
...
def get_relates_list(hero_name, one_hero_soup):
    ...
    for i in range(1, 4):
        selector = 'body > div.wrapper > div.zkcontent > div > div.zk-con4.zk-con > div.hero.ls.fl > div.hero-info-box > div > div:nth-child(' + str(
            i) + ') > '
        ...
        for item in href_url_lables:
            ...
            name = href_soup.select('body > div.wrapper > div.zk-con1.zk-con > div > div > div.cover > h2')[
                0].get_text()
            names.append(name)
...
def pretreatment():
    save_hero_info = pd.read_csv('./王者荣耀英雄基本信息.csv', encoding='GBK',
                                 names=['英雄名', '生成能力', '攻击伤害', '技能效果', '上手难度'])
    # 去空值
